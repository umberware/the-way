import { CORE, CoreCryptoService } from '../../../main';
...
describe('CoreCryptoService: ', () => {
    let cryptoService: CoreCryptoService;
    beforeAll(done => {
        ...
        import('../../resources/environment/main/main.test').then(
            () => {
                CORE.whenReady().subscribe(
                    () => {
                        cryptoService = CORE.getInstanceByName<CoreCryptoService>('CoreCryptoService') as CoreCryptoService;
                        done();
                    }
                );
            }
